...
public class RandomPointProvider {
    ...
    private static RandomPointProvider instance;

    private WeakReference<GenerateCallback> generateCallback;
    private ArrayList<InputPoint> points;

    public static RandomPointProvider getInstance() {
        if (instance == null) {
            instance = new RandomPointProvider();
        }
        return instance;
    }

    public void generate(GenerateCallback callback, GeographicDistribution geographicDistribution, int count) {
        this.points = null;
