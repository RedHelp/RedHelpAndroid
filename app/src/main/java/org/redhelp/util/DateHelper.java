...
public class DateHelper {
    ...
    public static String createSlotString(SlotsCommonFields slot, int index) {
        String start_time = DateHelper.getISTTimeInViewableFormat(slot.getStart_datetime());
        String end_time = DateHelper.getISTTimeInViewableFormat(slot.getEnd_datetime());
        String slot_string = String.format("%d.) %s - %s", index, start_time, end_time);
        return slot_string;
    }

    public static String getISTTimeInViewableFormat(String date_str) {
        Date start_datetime = DateTimeHelper.convertStringToJavaDate(date_str, JodaTimeFormatters.dateTimeFormatter);
        if(start_datetime == null)
            return null;
        DateFormat indianFormat =  new SimpleDateFormat("hh:mm a");
        indianFormat.setTimeZone(TimeZone.getTimeZone("Asia/Kolkata"));
        String newDateFormat = indianFormat.format(start_datetime);
